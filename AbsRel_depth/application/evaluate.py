...
class DepthDatasetEvaluation(object):
    ...
    def inverse_evaluate_all_datasets(self):
        ...
        dataset_list = ['Sintel','DIODE', 'ETH3D', 'Ibims', 'NYUV2', 'KITTI']
        method_list = ['rz_sb_mar']
        pro_dict = {'result_nb': [1.0]}

        for method in method_list:
            for dataset in dataset_list:
                for mode in mode_list:
                    gt_path = os.path.join('../Test_datasets', dataset, 'gt')
                    log_path = os.path.join('../Test_datasets', dataset, mode, 'logs_absrel.txt')
                    ...
                    for pro in pro_dict[mode]:
                        ...
                        if pro != 0.0:
                            ...
                            rmse = self.inverse_evaluate_depth_dataset(depth_path, gt_path, pro)
                            with open(log_path, 'a') as log_file:
                                log_file.write(
                                    method + '_' + str(pro) +
                                    ': RMSE=' + str(rmse) + '\n'
