...
class ChannelSELayer(torch.nn.Module):
    ...
    def __init__(self, num_channels):
        ...
        self.rezero_weight = torch.nn.Parameter(torch.tensor(0.0), requires_grad=True)
    ...
    def forward(self, inp):
        ...
        #  加入：Rezero
        out = self.rezero_weight * out + inp
        out = self.act(out)
...
class HANCLayer(torch.nn.Module):
    ...
    def __init__(self, in_chnl, out_chnl, k):
        ...
        if self.in_chnl == self.out_chnl:
            self.rezero_conv = None
        else:
            self.rezero_conv = torch.nn.Conv2d(self.in_chnl, self.out_chnl, 1, 1)
    ...
    def forward(self, inp):
        ...
        # 加入Rezero
        x = self.cnv(x)
        x = self.rezero_weight * x + (inp
                                      if self.rezero_conv is None else self.rezero_conv(inp))
        return x
...
class Conv2d_batchnorm(torch.nn.Module):
    ...
    def __init__(
        ...
        self.rezero_weight = torch.nn.Parameter(torch.tensor(0.0), requires_grad=True)
        self.in_chnl = num_in_filters
        self.out_chnl = num_out_filters
        if self.in_chnl == self.out_chnl:
            self.rezero_conv = None
        else:
            self.rezero_conv = torch.nn.Conv2d(self.in_chnl, self.out_chnl, 1, 1)
...
class Conv2d_channel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # 加入rezero
        x = self.rezero_weight * x + (x
                                      if self.rezero_conv is None else self.rezero_conv(x))
        # x = self.batchnorm(x)
        x = self.sqe(self.activation(x))
...
class ResPath(torch.nn.Module):
    ...
    def __init__(self, in_chnls, n_lvl):
        ...
        self.resweight = torch.nn.Parameter(torch.zeros(n_lvl), requires_grad=True)

    def forward(self, x):

        # for i in range(len(self.convs)):
        #     x = x + self.sqes[i](self.act(self.bns[i](self.convs[i](x))))
        # x = self.sqe(self.act(self.bn(x)))
        
        
        # 加入rezero
        for i in range(len(self.convs)):
            x = x + self.resweight[i] * self.sqes[i](self.act(self.bns[i](self.convs[i](x))))
        x = self.sqe(self.act(x))
        return x
...
class MLFC(torch.nn.Module):
    ...
    def __init__(self, in_filters1, in_filters2, in_filters3, in_filters4, lenn=1):
        ...
        for i in range(lenn):
            self.bns1.append(torch.nn.Identity())
            self.bns_mrg1.append(torch.nn.Identity())
            
            self.bns2.append(torch.nn.Identity())
            self.bns_mrg2.append(torch.nn.Identity())
            
            self.bns3.append(torch.nn.Identity())
            self.bns_mrg3.append(torch.nn.Identity())
            
            self.bns4.append(torch.nn.Identity())
            self.bns_mrg4.append(torch.nn.Identity())
        ...
        self.resweight1 = torch.nn.Parameter(torch.zeros(4), requires_grad=True)
        self.resweight2 = torch.nn.Parameter(torch.zeros(4), requires_grad=True)
        self.resweight3 = torch.nn.Parameter(torch.zeros(4), requires_grad=True)
        self.resweight4 = torch.nn.Parameter(torch.zeros(4), requires_grad=True)

        self.cnv_1 = torch.nn.ModuleList([])
        self.cnv_2 = torch.nn.ModuleList([])
        self.cnv_3 = torch.nn.ModuleList([])
        self.cnv_4 = torch.nn.ModuleList([])
        
        for i in range(lenn):
            self.cnv_1.append(torch.nn.Conv2d(480, 32, 1, 1))
            self.cnv_2.append(torch.nn.Conv2d(480, 64, 1, 1))
            self.cnv_3.append(torch.nn.Conv2d(480, 128, 1, 1))
            self.cnv_4.append(torch.nn.Conv2d(480, 256, 1, 1))
    def forward(self, x1, x2, x3, x4):
        ...
        for i in range(len(self.cnv_blks1)):
            ...
            x_c1 = self.cnv_blks1[i](x_c1)
            
            x_c1 = self.bns1[i](x_c1)
            
            
            x_c1 = self.resweight1[i] * x_c1 + self.cnv_1[i](torch.cat(
                            [
                                x1,
                                self.no_param_up(x2),
                                self.no_param_up(self.no_param_up(x3)),
            ...
            x_c2 = self.cnv_blks2[i](x_c2)
            
            x_c2 = self.bns2[i](x_c2)
            x_c2 = self.act(x_c2)
            
            x_c2 = self.resweight2[i] * x_c2 + self.cnv_2[i](torch.cat(
                            [
                                self.no_param_down(x1),
                                (x2),
                                (self.no_param_up(x3)),
            ...
            x_c3 = self.cnv_blks3[i](x_c3)
            
            x_c3 = self.bns3[i](x_c3)
            x_c3 = self.act(x_c3)
            
            x_c3 = self.resweight3[i] * x_c3 + self.cnv_3[i](torch.cat(
                            [
                                self.no_param_down(self.no_param_down(x1)),
                                self.no_param_down(x2),
                                (x3),
            ...
            x_c4 = self.cnv_blks4[i](x_c4)
            
            x_c4 = self.bns4[i](x_c4)
            x_c4 = self.act(x_c4)
            
            x_c4 = self.resweight4[i] * x_c4 + self.cnv_4[i](torch.cat(
                            [
                                self.no_param_down(self.no_param_down(self.no_param_down(x1))),
                                self.no_param_down(self.no_param_down(x2)),
                                self.no_param_down(x3),
...
class ACC_UNet(torch.nn.Module):
    ...
    def __init__(self, n_channels, n_classes, n_filts=32, sigmoid=False):
        """
        Initialization
