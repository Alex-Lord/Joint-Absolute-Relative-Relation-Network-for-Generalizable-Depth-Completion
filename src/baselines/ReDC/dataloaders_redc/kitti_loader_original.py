...
def get_paths_and_transform(split, args):
    ...
    if split == "train_val":
        ...
        glob_d = os.path.join(
            args.data_folder,
            'data_depth_velodyne/*/*_sync/proj_depth/velodyne_raw/image_0[2,3]/*.png'
        )
        glob_gt = os.path.join(
            args.data_folder,
            'data_depth_annotated/*/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png'
        )
...
def get_rbf_paths(p):
    ps = p.split('/')
    pnew = '/'.join([sequoia_kitti_path] + ps[-7:-4] +
                    ['rbf'] + ps[-2:-1]+ [ps[-1][:-4] + 
                    '.tiff'])
    return pnew

def get_nearest_path(path):
    ps = path.split('/')
    ps[-1] = 'nearest_'+ps[-1]
    ps[-1] = ps[-1].replace(".png", ".tiff")
    ps[-7] = 'nearest_data_depth_velodyne'
    new_path = '/'.join(ps)
    return new_path

def get_pe_path(path):
    ps = path.split('/')
    ps[-1] = 'pe_'+ps[-1]
    ps[-1] = ps[-1].replace(".png", ".tiff")
    ps[-7] = 'pe_data_depth_velodyne'
    new_path = '/'.join(ps)
    return new_path


sequoia_kitti_path = '/sequoia/data2/alexsun/dataset/kitti/data'
class KittiDepth(data.Dataset):
    ...
    def __getraw__(self, index):
        ...
        if self.args.rbf:
            rbf_path = get_rbf_paths(self.paths['d'][index]) if \
            (self.paths['d'][index] is not None and self.args.use_d) else None
            sparse = depth_read(rbf_path)
#             print(rbf_path)
        elif self.args.nearest:
            nearest_path = get_nearest_path(self.paths['d'][index]) if \
            (self.paths['d'][index] is not None and self.args.use_d) else None
            sparse = depth_read(nearest_path)
        elif self.args.pe:
            pe_path = get_pe_path(self.paths['d'][index]) if \
            (self.paths['d'][index] is not None and self.args.use_d) else None
            sparse = depth_read(pe_path)
    ...
    def __getitem__(self, index):
        ...
        if sparse is None:
            sparse = np.zeros(target.shape)
        if target is None:
            target = np.zeros(sparse.shape)
        ...
        items['gt'] = items['gt'].unsqueeze(0)
        items['d'] = items['d'].unsqueeze(0)
        
        items['path'] = self.paths['d'][index]
        return items
