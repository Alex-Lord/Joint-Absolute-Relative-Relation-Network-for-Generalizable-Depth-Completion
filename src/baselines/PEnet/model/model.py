...
import math
import sys
sys.path.append('/data/8T/cby/Trans_G2/src/baselines/PEnet/model')
import CoordConv
...
class GeometryFeature(nn.Module):
    ...
    def forward(self, z, vnorm, unorm, h, w, ch, cw, fh, fw):
        ...
        # z_device = check_device(z)
        # vnorm_device = check_device(vnorm)
        # unorm_device = check_device(unorm)
        # # ch_device = check_device(ch)
        # # cw_device = check_device(cw)
        # # fh_device = check_device(fh)
        # # fw_device = check_device(fw)

        # print("z:", z_device)
        # print("vnorm:", vnorm_device)
        # print("unorm:", unorm_device)
        # # print("ch:", ch_device)
        # # print("cw:", cw_device)
        # # print("fh:", fh_device)
        # # print("fw:", fw_device)
...
class PENet_C2(nn.Module):
    ...
    def forward(self, rgb, d, K):
        ...
        feature_s1, feature_s2, coarse_depth = self.backbone(rgb, d, K)
        depth = coarse_depth
